...
Current implementation head is commit `0b8dc507522596cc9ba8659b56cbb96744f7c375`. M0 is not complete
or accepted. Signed endpoint-catalog semantic verification, bounded length-prefixed Unix framing and
the rate admission application now join endpoint policy, operation facts, causal capability and
`SO_PEERCRED` before authority calls. The next unit is the PostgreSQL adapter and normalized
multi-class runtime-policy ingestion, then the executable bounded rate service. The locked gateway
...
Expected: CI passes 41 unit, 3 property, 2 contract and 2 security tests; migrations pass both
