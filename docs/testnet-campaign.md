...
实际结果偏离。当前 0.1 USDT 最大净损失预算对应首笔 SOLUSDT 约 0.97% 的价格距离；该
笔并未触发止损，而是在 30 秒最大持仓时间退出。正式策略必须用 setup 的结构锚点生成
...
2026-07-14 的首个三币样本同时运行 SOLUSDT、BNBUSDT 和 XRPUSDT。三单均在 30 秒到期
