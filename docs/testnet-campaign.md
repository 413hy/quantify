...
该服务只连接 Binance USDⓈ-M Futures Testnet，不连接生产交易端点。V4.8 固定使用
...
- 状态文件持续记录 `last_signal_diagnostics` 和累计 `signal_gate_counts`，区分历史不足、市场
  ...
  “0 交易”却无法定位具体门控；V4.8 另记录 `last_confirmation_diagnostics` 和
  `confirmation_gate_counts`，继续区分计划后的质量、活跃度样本、活跃度倍率、连续确认和已确认；
- V4.8 取前 10 根已闭合 1 分钟 K 线收盘价，使用最小二乘直线预测后 10 分钟收盘价，再对
  前后共 20 个价格取平均。预测均价只作为方向门控：相对当前中间价的方向化预测必须与交易
  信号同向且至少 1 bps；反向预测直接拒绝，不再包装成均值回归入场。通过门控后，做多限价
  ...
  因此 V4.8 删除了这层未经数据支持的额外价差。部分成交后立即
...
- 最新有效信号与持仓反向时，唯一执行器先撤销旧保护并市价平掉旧方向，将退出原因记录为
  `SIGNAL_REVERSAL`，然后绕过逐币平仓冷却直接提交已经确认的新方向。反向新仓仍使用预测限价，
  仍受每日次数、每日净亏损、费用后目标和所有保护门控约束；低质量或未确认信号无权接管仓位；
...
uv run python scripts/review-testnet-results.py \
  ...
  --strategy TESTNET_EXPERIMENT_OF_PA_V4_8
```
